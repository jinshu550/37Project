...
namespace SerializeReferenceEditor.Editor.Processing
{
    public class ProcessingCoordinator : AssetPostprocessor
    {
        ...
        // 新增：Unity内置资源路径标识
        private const string UnityBuiltInExtraPath = "unity_builtin_extra";
        ...
        // 检查路径是否为Unity内置资源
        private static bool IsUnityBuiltInResource(string path)
        {
            return !string.IsNullOrEmpty(path) && path.Contains(UnityBuiltInExtraPath);
        }
    }
}
