...
public class Perk
{
    ...
    private void Reaction(GameAction gameAction)
    {
        ...
        {
            if (data.TargetCardTitles.Contains(playCardGA.Card.Title) && condition.SubConditionIsMet(gameAction))
            {
                AddAditionalEffect(playCardGA);
                return;
            }
        }
        else if (condition.SubConditionIsMet(gameAction))
        {
    ...
    private void AddAditionalEffect(PlayCardGA playCardGA)
    {
        if (data.AdditionalEffect == null) return;
        List<CombatantView> targets = data.AdditionalEffect.TargetMode.GetTargets();
        PerformEffectGA additionalEffectGA = new PerformEffectGA(
            data.AdditionalEffect.Effect,
            targets
        );
        ActionSystem.Instance.AddReaction(additionalEffectGA);
    }
}
